...
export class WeatherResultsComponent implements OnInit {
  ...
  setDailyChartOptions() {
    const dates = this.forecast.slice(0, 5).map((entry: any) => new Date(entry.dt * 1000).toLocaleDateString());
    const temperatures = this.forecast.slice(0, 5).map((entry: any) => entry.temp.day);
